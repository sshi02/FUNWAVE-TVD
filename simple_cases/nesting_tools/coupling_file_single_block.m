...
% input
foutput='/Volumes/DISK_2020_5/NTHMP_Wilmington/Results/PRT_GridB_NC/';
...
fwriteout='/Volumes/DISK_2020_5/NTHMP_Wilmington/Results/PRT_GridB_NC/';

coupling_info=load([fgrid 'station_WI.txt']);
num_blocks=coupling_info(end,3)/4;

iscale=[18];  % spacial refine scale for all blocks
itscale = [2]; % coarsen time scale, skip x output time steps

starttime  = [12000];   % in seconds, adjust to truncate time series
stoptime=starttime+18000.0; % 5 hours

%stoptime   = [11000];% Inf; % in sec. indicate Inf to include all.

coupling_filename={'coupling_PRT_WI.txt'}

logfile='log_PRT_WI.txt';
% ----
...
filename=[fwriteout coupling_filename{kk}];
FIN = fopen(filename,'w');           

fprintf(FIN,'coupling data\nboundary info: num of points, start point');
fprintf(FIN,'\nEAST\n\t%d\t\t%d',size(eta_fine{1},2),1);
fprintf(FIN,'\nWEST\n\t%d\t\t%d',size(eta_fine{2},2),1);
fprintf(FIN,'\nSOUTH\n\t%d\t\t%d',size(eta_fine{3},2),1);
fprintf(FIN,'\nNORTH\n\t%d\t\t%d',size(eta_fine{4},2),1);
fprintf(FIN,'\nTIME SERIES');
for t = 1:length(time)
